...
ggplot(world_df_long, aes(area = source_emissions, fill = sector, label = source, subgroup = sector)) +
  geom_treemap() +
  geom_treemap_text(color = "white", place = "centre", grow = TRUE, reflow = TRUE, family = "barlow") +
  scale_fill_manual(values = custom_colors) +
  labs(title = "What sources emitted the most methane in 2015?") +
  theme(legend.position = "top",
      legend.title = element_blank(), 
      legend.text = element_text(size = 10, family = "barlow", face = "bold"),
      plot.title = element_text(family = "merri sans", face = "bold", size = 12, hjust = 0.5))
```
...
ggplot(top_countries, aes(x = population, y = emissions_pc)) +
  ...
  geom_text(data = top_countries,
            aes(label = country, x = population, y = emissions_pc, size = total_emissions, color = country), hjust = -0.25, vjust = 2, size = 3, family = "barlow", fontface = "bold") +
  scale_x_continuous(limits = c(0, 1500000000)) +
  labs(x = "Population", y = "Per Capita Emissions (MtCO2eq)") +
  labs(title = "53% of global methane emissions in 2015 came from\n5 highest-emitting countries and the European Union (27 countries)") + # format colored HTML text in subtitle
  theme_minimal() +
  theme(panel.grid.major = element_blank(), # remove major gridlines
        panel.grid.minor = element_blank()) + # removes minor gridlines
  theme(axis.title.x = element_text(family = "barlow", face = "bold", size = 10, color = "grey30"),
        axis.title.y = element_text(family = "barlow", face = "bold", size = 10, color = "grey30"),
        axis.text.x = element_text(family = "barlow", face = "bold"),
        axis.text.y = element_text(family = "barlow", face = "bold")) +
  theme(axis.line = element_line(color = "black", size = 0.5)) + # add axis line
  theme(plot.title = element_text(family = "merri sans", size = 12, hjust = 0.5)) +
  ...
  annotate(
    ...
    hjust = "left",
    size = 4,
    family = "merri sans") + # create text annotation
  annotate(
    geom = "curve",
...
ggplot(top_countries_long, aes(x = country, y = emissions, fill = sector)) +
  ...
  labs(y = "Emissions (MtCO2eq)", title = "Sources of emissions in 5 highest-emitting\ncountries and the European Union", fill = "Sector") +
  scale_fill_manual(values = custom_colors) +
  theme_minimal() +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_text(family = "barlow", face = "bold", color = "grey30"),
        axis.text.x = element_text(family = "barlow", face = "bold"),
        axis.text.y = element_text(family = "barlow", face = "bold"),
        legend.position = c(0.85, 0.85),
        legend.text = element_text(color = "grey30", face = "bold", size = 9, family = "barlow"),
        legend.title = element_blank(),
        plot.title = element_text(family = "merri sans", size = 12, hjust = 0.5))
```
